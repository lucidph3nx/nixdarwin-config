...
}:
{
  config =
    lib.mkIf
      (
        config.homeManagerModules.prism.contextSwitcher.enable
        # no point in installing if tmux is not
        && config.homeManagerModules.prism.tmux.enable
      )
      {
              ...
              def create_or_switch_session(selected_path, project_root=None):
                  ...
                  if not session_exists:
                      ...
                      if is_scratchpad:
                          ...
                          agentEnvPrefix = "${config.homeManagerModules.prism._internal.agentEnvPrefix}"
                          subprocess.run(
